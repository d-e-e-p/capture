...
void SelectFrameSize(IControl* frmsizControl, IControl* frmintControl)
{
    int menuSizeFrmsiz;
    int menuSizeFrmint;
    int i;
    ...
    for (i = 0; i < menuSizeFrmsiz; i++) {
        ...
        sv_control_Set(frmsizControl, menufrmsiz[i].index);
        printf("%d - %s", menufrmsiz[i].index, menufrmsiz[i].name);
        
        CMenuEntryList menu = sv_control_GetMenuEntries(frmintControl, &menuSizeFrmint);
        if (menuSizeFrmint != 0) {
            printf(" %s fps", menu[0].name);
        }
