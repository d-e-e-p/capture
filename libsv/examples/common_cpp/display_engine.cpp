...
void DisplayEngine::Construct(const ConfiguredCameras &cameras)
{
    ...
    for (auto camera : cameras) {
        imagePipelines.push_back(std::unique_ptr<ImagePipeline>(new ImagePipeline(camera.camera)));
...
void DisplayEngine::ConstructHotkeyActions()
{
    ...
    hotkeyActions.emplace_back(HotkeyAction(
        {'q', 'Q', SpecialHotkey::ENTER, SpecialHotkey::ESC},
        "Exit application",
...
void DisplayEngine::SaveImage(cv::UMat &image, std::string name)
{
    switch(saveImage) {
        case SaveImageOptions::JPEG:
            processor.SaveImageAsJpeg(image, name);
            break;
        case SaveImageOptions::PNG:
            processor.SaveImageAsPng(image, name);
            break;
        case SaveImageOptions::TIFF:
            processor.SaveImageAsTiff(image, name);
            break;
        default:
            break;
    }
