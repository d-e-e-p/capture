...
class Imagedata{
 public:
    ...
    void loadImage(fs::path file) {                                                                

        src = file;

        // get data in
        size = fs::file_size(src);
        ...
        LOGV << "loaded " << src.string() << " size:" << size
    ...
    void getExif() {
        ...
        TagInfo *info = exiftool->ImageInfo(src.string().c_str(),"-UniqueCameraModel");
        if (info) {
    ...
    void writeAnnotated( fs::path to, bool label) {
        ...
        if (label) {
            ...
            // 728 x 544 -> 768 x 584
            cmd = "magick  " + src.string() + " -clahe 25x25%+128+2 -gravity Southwest -background black -extent 768x584 -font Inconsolata -pointsize " + pointsize_north + " -fill yellow -gravity North -annotate 0x0+0+15 \"" + text_north + "\" -gravity East -pointsize " + pointsize_east + " -annotate 90x90+20+232 \"" + text_east + "\" " + dst.string();
        } else {
            cmd = "magick  " + src.string() + " -clahe 25x25%+128+2 " + dst.string();
        }
